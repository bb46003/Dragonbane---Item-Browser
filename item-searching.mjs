...
export class itemsSearch extends foundry.applications.api.ApplicationV2 {
  ...
  async changeitemType(event) {
    ...
    this.filterData.chosenItems = await this.itemFiltration(
      this.filterData,
      this.filterData.chosenType,
    );
  ...
  async _prepareWorldsItems(chosenType, actorID, actorType) {
    ...
    let filteredItems = await Promise.all(
      game.items.map(async (item) => {
        ...
        const isSupplyTypeValid = item.system.supply
          ? supplyTypes.includes(item.system.supply)
          : true;
        ...
        const willBeAvaliable =
          isTypeValid && isSupplyTypeValid && isInSkippedFolder;
    ...
    const weaponsSkillsArray = game.items.filter(
      (item) => item.type === "skill" && item.system.skillType === "weapon",
    );
    ...
    const removeDuplicatedMagicSkills = [
      ...new Set(magicSkills.map((item) => item.system.school)),
    ];
    const magicSkillsmNames = removeDuplicatedMagicSkills.reduce(
      (obj, name) => {
        obj[name] = name;
        return obj;
      },
  ...
  async getFolderPathIds(folder) {
    const skippedFolders =
      game.settings.get("dragonbane-item-browser", "selectedFolders") || [];
    ...
    for (const parent of parentFoldersArray) {
      if (
        skippedFolders.includes(parent.id) ||
        skippedFolders.includes(folder)
      ) {
        return false;
      }
    }
    ...
    switch (chosenType) {
      ...
      case "weapon":
        chosenItems = data.items.filter(
          (item) =>
            item.type === "weapon" &&
            (data.filters.weaponSkill === "any" ||
              item.system.skill.name === data.filters.weaponSkill) &&
            (data.filters.grip === "any" ||
              item.system.grip.value === data.filters.grip) &&
            (data.filters.weaponFeature === "any" ||
              item.system.features.includes(data.filters.weaponFeature)) &&
            (data.filters.supply === "any" ||
              item.system.supply === data.filters.supply),
        );
        break;
      case "spell":
        chosenItems = data.items.filter(
          (item) =>
            item.type === "spell" &&
            (data.filters.school === "any" ||
              item.system.school === data.filters.school) &&
            (data.filters.duration === "any" ||
              item.system.duration === data.filters.duration) &&
            (data.filters.castingTime === "any" ||
              item.system.castingTime === data.filters.castingTime) &&
            (data.filters.range === "any" ||
              item.system.rangeType === data.filters.range) &&
  ...
  async addItem(event) {
    ...
    await actor.update();
    ChatMessage.create({
      content: game.i18n.format("DB-IB.addItem", {
        actor: actor.name,
        type: item.type,
        item: item.name,
      }),
  ...
  async spendMoney(item, actor) {
    ...
    if (
      (actorGC !== actor.system.currency.gc ||
        actorSC !== actor.system.currency.sc ||
        actorCC !== actor.system.currency.cc) &&
      enoughMoney
    ) {
      await actor.update({
      ...
      ChatMessage.create({
        content: game.i18n.format("DB-IB.manualMonyRemoval", {
          itemPrice: itemPrice,
          item: item.name,
        }),
        speaker: ChatMessage.getSpeaker({ actor }),
  ...
  async itemFilter(event) {
    ...
    const chosenItem = await this.itemFiltration(
      filterData,
      filterData.chosenType,
    );
    const filteredItems = chosenItem.filter((item) =>
    ...
    setTimeout(() => {
      ...
      if (newInputField) {
        ...
        newInputField.setSelectionRange(
...
async function addBuyButton(
  ...
  if (sucess && !isDragon) {
    ...
    const regularPrice = `<br><p class="merchant-text"> ${tekst}</p>`;
