import {
  ...
  sellingItem,
  addChatListeners,
} from "./item-searching.mjs";
import * as DBIBChat from "./item-searching.mjs";
import {
  merchant,
  merchantData,
  sellingItemMerchat,
} from "./merchant-character.mjs";
...
Hooks.on("hoverToken", async (token, ev) => {
  ...
  const temporaryOwner =
    actor.flags["dragonbane-item-browser"]?.temporary || false;
  if (
    actor.ownership[game.user._id] === 3 &&
    actor.type === "dragonbane-item-browser.merchant" &&
    !temporaryOwner
...
Hooks.on("renderSettingsConfig", (app, html, data) => {
  ...
  const $html = $(html);
  const barterSkillRow = $html
    .find('[name="dragonbane-item-browser.custom-barter-skill"]')
    .closest(".form-group");
  ...
  const label = $html.find(
    'label[for="settings-config-dragonbane-item-browser.skip-folders-for-browser"]',
  );
  let settingContainer;
  if (game.release.generation < 13) {
    settingContainer = html.find(
      '[data-setting-id="dragonbane-item-browser.skip-folders-for-browser"] .form-fields',
    );
  } else {
  ...
  const barterRollCheckbox = $html.find(
    '[name="dragonbane-item-browser.barter-roll-when-buys"]',
  );
  barterRollCheckbox.on("change", (event) => {
    toggleCustomBarterSkill(event.target.checked);
  });

  const skipFoldersCheckbox = $html.find(
    '[name="dragonbane-item-browser.skip-folders-for-browser"]',
  );
...
Hooks.on("renderDoDCharacterSheet", async (html) => {
  ...
  creatItemButton.forEach((button) => {
    ...
    if (dataType !== "effect") {
      const existingButton =
        button.nextElementSibling?.classList.contains("item-browser");
  ...
  const sellsSetting = game.settings.get(
    "dragonbane-item-browser",
    "sell-items",
  );
